...
if (!empty($_POST)){
    ...
    if(empty($event_id) || empty($event_title) || empty($group_id) ){
        ...
        if ($sql){
            ?>
            <p class="WordPress_ok"><?php echo _e('Event was successfully updated.','viavi-wp-timeline'); ?></p>
            <?php
        }else{
            ?>
            <p class="WordPress_hata"><?php echo _e('An error occurred while updating this event.','viavi-wp-timeline'); ?></p>
            <?php
        }                
    }
}
