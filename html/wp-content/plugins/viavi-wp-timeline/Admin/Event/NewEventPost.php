...
if (!empty($_POST)){
    ...
    if(empty($event_title) || empty($group_id) ){
        ...
        $sql=mysql_query('insert into '.WordPress_WP_TIMELINE_DB_PREFIX.'WordPress_Timeline (group_id,timeline_bc,timeline_date,title,event_content,type) values ("'.$group_id.'","'.$sql_bc_colon.'","'.$sql_datetime_colon.'","'.$event_title.'","'.$event_content.'","event")');
